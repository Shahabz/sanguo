...
int city_load_auto( LPCB_GETCITY pCB_GetCity, LPCB_LOADCITY pCB_LoadCity, const char *pTab )
{
	...
	while( ( row = mysql_fetch_row( res ) ) )
	{
		...
		pCity->vote = atoi(row[offset++]);
		pCity->ballot = atoi(row[offset++]);
		pCity->tokenballot = atoi(row[offset++]);
		memcpy( pCity->soldiers, row[offset++], sizeof(int)*3 );
		memcpy( pCity->tc_level, row[offset++], sizeof(char)*2 );
		memcpy( pCity->tc_progress, row[offset++], sizeof(char)*2 );
		pCity->bufftrain = atoi(row[offset++]);
		pCity->teacherid = atoi(row[offset++]);
		pCity->act06_state = atoi(row[offset++]);
		pCity->robot_ai = atoi(row[offset++]);
		pCity->robot_cd = atoi(row[offset++]);
		memcpy( pCity->edevent, row[offset++], sizeof(char)*16 );
		pCity->edevent_sec = atoi(row[offset++]);
		pCity->edevent_num = atoi(row[offset++]);
		pCity->act12_state = atoi(row[offset++]);
		pCity->act12_turn = atoi(row[offset++]);
		pCity->act12_idx = atoi(row[offset++]);
		pCity->act33_pay = atoi(row[offset++]);
		memcpy( pCity->edquest, row[offset++], sizeof(int)*16 );
		pCity->edquest_isget = atoi(row[offset++]);
		pCity->act22kill = atoi(row[offset++]);
		if( pCB_LoadCity )
