...
int city_load_auto( LPCB_GETCITY pCB_GetCity, LPCB_LOADCITY pCB_LoadCity, const char *pTab )
{
	...
	while( ( row = mysql_fetch_row( res ) ) )
	{
		...
		pCity->tc_state = atoi(row[offset++]);
		pCity->tc_kind = atoi(row[offset++]);
		pCity->tc_num = atoi(row[offset++]);
		pCity->tc_tech = atoi(row[offset++]);
