...
int city_load_auto( LPCB_GETCITY pCB_GetCity, LPCB_LOADCITY pCB_LoadCity, const char *pTab )
{
	...
	while( ( row = mysql_fetch_row( res ) ) )
	{
		...
		memcpy( pCity->nequip_level, row[offset++], sizeof(short)*6 );
		memcpy( pCity->nequip_exp, row[offset++], sizeof(char)*6 );
		pCity->nequip_kind = atoi(row[offset++]);
		pCity->nequip_pro = atoi(row[offset++]);
...
int city_save_auto( City *pCity, const char *pTab, FILE *fp )
{
	...
	char szText_nequip_level[sizeof(short)*6*2+1]={0};
	char szText_nequip_exp[sizeof(char)*6*2+1]={0};
