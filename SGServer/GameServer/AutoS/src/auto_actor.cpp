...
int actor_load_auto( int actorid, Actor *pActor, const char *pTab )
{
	...
	sprintf( szSQL, "select `actorid`,`admin`,`lastip`,`createtime`,`forbidtime`,`name`,`nation`,`shape`,`level`,`token`,`total_charge`,`charge_dollar`,`itemext`,`equipext`,`quest_talkid`,`worldquest_complete`,`sflag`,`fdate`,`today_char`,`today_int0`,`today_int1`,`today_int2`,`today_int3`,`today_int4`,`today_int5`,`today_int6`,`today_int7`,`cd0`,`cd1`,`cd2`,`cd3`,`cd4`,`cd5`,`cd6`,`cd7`,`config`,`function`,`storyid`,`story_star`,`story_hero`,`story_resnum`,`story_restime`,`story_resreset`,`story_itemnum`,`story_drawing`,`story_sweephero`,`lastpeople_c`,`lastpeople_n`,`cdkey1`,`cdkey2`,`cdkey3`,`cdkey4`,`hv_ln`,`hv_hn`,`hv_pro` from %s where actorid='%d'", pTab, actorid );
...
RE_ACTOR_UPDATE:
	sprintf( szSQL, "REPLACE INTO %s (`actorid`,`lastip`,`createtime`,`forbidtime`,`name`,`nation`,`shape`,`level`,`token`,`total_charge`,`charge_dollar`,`itemext`,`equipext`,`quest_talkid`,`worldquest_complete`,`sflag`,`fdate`,`today_char`,`today_int0`,`today_int1`,`today_int2`,`today_int3`,`today_int4`,`today_int5`,`today_int6`,`today_int7`,`cd0`,`cd1`,`cd2`,`cd3`,`cd4`,`cd5`,`cd6`,`cd7`,`config`,`function`,`storyid`,`story_star`,`story_hero`,`story_resnum`,`story_restime`,`story_resreset`,`story_itemnum`,`story_drawing`,`story_sweephero`,`lastpeople_c`,`lastpeople_n`,`cdkey1`,`cdkey2`,`cdkey3`,`cdkey4`,`hv_ln`,`hv_hn`,`hv_pro`) Values('%d','%s','%d','%d','%s','%d','%d','%d','%d','%d','%f','%d','%d','%d','%d','%d','%d','%s','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%s','%d','%d','%s','%s','%s','%s','%s','%s','%s','%s','%d','%d','%d','%d','%d','%d','%d','%d','%d')",pTab,pActor->actorid,db_escape((const char *)pActor->lastip,szText_lastip,0),pActor->createtime,pActor->forbidtime,db_escape((const char *)pActor->name,szText_name,0),pActor->nation,pActor->shape,pActor->level,pActor->token,pActor->total_charge,pActor->charge_dollar,pActor->itemext,pActor->equipext,pActor->quest_talkid,pActor->worldquest_complete,pActor->sflag,pActor->fdate,db_escape((const char *)pActor->today_char,szText_today_char,sizeof(char)*128),pActor->today_int[0],pActor->today_int[1],pActor->today_int[2],pActor->today_int[3],pActor->today_int[4],pActor->today_int[5],pActor->today_int[6],pActor->today_int[7],pActor->cd[0],pActor->cd[1],pActor->cd[2],pActor->cd[3],pActor->cd[4],pActor->cd[5],pActor->cd[6],pActor->cd[7],db_escape((const char *)pActor->config,szText_config,sizeof(char)*8),pActor->function,pActor->storyid,db_escape((const char *)pActor->story_star,szText_story_star,sizeof(char)*128),db_escape((const char *)pActor->story_hero,szText_story_hero,sizeof(char)*32),db_escape((const char *)pActor->story_resnum,szText_story_resnum,sizeof(char)*32),db_escape((const char *)pActor->story_restime,szText_story_restime,sizeof(int)*32),db_escape((const char *)pActor->story_resreset,szText_story_resreset,sizeof(char)*32),db_escape((const char *)pActor->story_itemnum,szText_story_itemnum,sizeof(char)*64),db_escape((const char *)pActor->story_drawing,szText_story_drawing,sizeof(char)*16),db_escape((const char *)pActor->story_sweephero,szText_story_sweephero,sizeof(short)*4),pActor->lastpeople_c,pActor->lastpeople_n,pActor->cdkey1,pActor->cdkey2,pActor->cdkey3,pActor->cdkey4,pActor->hv_ln,pActor->hv_hn,pActor->hv_pro);
	if( fp )
	{
		fprintf( fp, "%s;\n", szSQL );
	}
