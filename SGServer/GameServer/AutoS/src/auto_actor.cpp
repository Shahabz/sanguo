...
int actor_load_auto( int actorid, Actor *pActor, const char *pTab )
{
	...
	if( ( row = mysql_fetch_row( res ) ) )
	{
		...
		pActor->fs_visit = atoi(row[offset++]);
		pActor->fs_step = atoi(row[offset++]);
		pActor->fs_back = atoi(row[offset++]);
	}
	else
	{
