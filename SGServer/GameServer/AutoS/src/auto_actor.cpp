...
int actor_load_auto( int actorid, Actor *pActor, const char *pTab )
{
	...
	if( ( row = mysql_fetch_row( res ) ) )
	{
		...
		pActor->teacher = atoi(row[offset++]);
		memcpy( pActor->teacheraward, row[offset++], sizeof(char)*10 );
		pActor->teachershop = atoi(row[offset++]);
	}
	else
	{
