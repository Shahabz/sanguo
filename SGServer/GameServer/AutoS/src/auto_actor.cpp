...
int actor_load_auto( int actorid, Actor *pActor, const char *pTab )
{
	...
	if( ( row = mysql_fetch_row( res ) ) )
	{
		...
		pActor->charge_dollar = atoi(row[offset++]);
		...
		pActor->act17_state = atoi(row[offset++]);
		pActor->coliseum_maxrank = atoi(row[offset++]);
		pActor->coliseum_rank = atoi(row[offset++]);
		pActor->coliseum_serieswin = atoi(row[offset++]);
		pActor->coliseum_winnum = atoi(row[offset++]);
		pActor->coliseum_losenum = atoi(row[offset++]);
		memcpy( pActor->coliseum_team, row[offset++], sizeof(short)*3 );
		pActor->edsignin = atoi(row[offset++]);
		pActor->edsignin_fmonth = atoi(row[offset++]);
		pActor->edsignin_pro = atoi(row[offset++]);
	}
	else
	{
