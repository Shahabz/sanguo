...
RE_ACTOR_UPDATE:
	sprintf( szSQL, "REPLACE INTO %s (`actorid`,`lastip`,`createtime`,`forbidtime`,`name`,`nation`,`shape`,`level`,`token`,`total_charge`,`charge_dollar`,`itemext`,`equipext`,`quest_talkid`,`worldquest_complete`,`blacklist`,`sflag`,`fdate`,`today_char`,`today_short`,`today_int0`,`today_int1`,`today_int2`,`today_int3`,`today_int4`,`today_int5`,`today_int6`,`today_int7`,`cd0`,`cd1`,`cd2`,`cd3`,`cd4`,`cd5`,`cd6`,`cd7`,`config`,`permission`,`storyid`,`story_star`,`story_hero`,`story_restime`,`story_resnum`,`story_resreset`,`story_itemnum`,`story_drawing`,`story_sweephero`,`lastpeople_c`,`lastpeople_n`,`cdkey1`,`cdkey2`,`cdkey3`,`cdkey4`,`hv_ln`,`hv_hn`,`hv_pro`,`vipbag`,`shop_useitem`,`charge_point`,`pay_maxtier`,`shop_saleitem`,`neq_crit`,`mapcallfree`,`wishingid`,`wishingday`,`wishingopen`,`wishingcd`,`shape_bag`,`student`,`te_award`,`te_awarded`,`te_shop`,`act02_state`,`act03_state`,`act04_state`,`act04_value`,`act05_item`,`act05_isbuy`,`act05_buynum`,`act05_xw`,`act05_upnum`,`act05_upstamp`,`act08_state`,`fs_nodeid`,`fs_awardkind`,`fs_awardnum`,`fs_awardfday`,`fs_weekkind`,`fs_weeknum`,`fs_fweek`,`girlshop`,`girlshop_buy`,`girlshop_fday`,`maidname`) Values('%d','%s','%d','%d','%s','%d','%d','%d','%d','%d','%f','%d','%d','%d','%d','%s','%d','%d','%s','%s','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%s','%d','%d','%s','%s','%s','%s','%s','%s','%s','%s','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%s','%s','%d','%s','%d','%s','%d','%d','%s','%s','%s','%d','%d','%d','%s','%s','%s','%d','%d','%d','%d','%d','%d','%d','%s','%s','%d','%s','%s','%d','%s','%d','%d','%s')",pTab,pActor->actorid,db_escape((const char *)pActor->lastip,szText_lastip,0),pActor->createtime,pActor->forbidtime,db_escape((const char *)pActor->name,szText_name,0),pActor->nation,pActor->shape,pActor->level,pActor->token,pActor->total_charge,pActor->charge_dollar,pActor->itemext,pActor->equipext,pActor->quest_talkid,pActor->worldquest_complete,db_escape((const char *)pActor->blacklist,szText_blacklist,sizeof(int)*50),pActor->sflag,pActor->fdate,db_escape((const char *)pActor->today_char,szText_today_char,sizeof(char)*128),db_escape((const char *)pActor->today_short,szText_today_short,sizeof(short)*32),pActor->today_int[0],pActor->today_int[1],pActor->today_int[2],pActor->today_int[3],pActor->today_int[4],pActor->today_int[5],pActor->today_int[6],pActor->today_int[7],pActor->cd[0],pActor->cd[1],pActor->cd[2],pActor->cd[3],pActor->cd[4],pActor->cd[5],pActor->cd[6],pActor->cd[7],db_escape((const char *)pActor->config,szText_config,sizeof(char)*8),pActor->permission,pActor->storyid,db_escape((const char *)pActor->story_star,szText_story_star,sizeof(char)*128),db_escape((const char *)pActor->story_hero,szText_story_hero,sizeof(char)*32),db_escape((const char *)pActor->story_restime,szText_story_restime,sizeof(int)*32),db_escape((const char *)pActor->story_resnum,szText_story_resnum,sizeof(char)*32),db_escape((const char *)pActor->story_resreset,szText_story_resreset,sizeof(char)*32),db_escape((const char *)pActor->story_itemnum,szText_story_itemnum,sizeof(char)*64),db_escape((const char *)pActor->story_drawing,szText_story_drawing,sizeof(char)*16),db_escape((const char *)pActor->story_sweephero,szText_story_sweephero,sizeof(short)*4),pActor->lastpeople_c,pActor->lastpeople_n,pActor->cdkey1,pActor->cdkey2,pActor->cdkey3,pActor->cdkey4,pActor->hv_ln,pActor->hv_hn,pActor->hv_pro,pActor->vipbag,pActor->shop_useitem,pActor->charge_point,pActor->pay_maxtier,db_escape((const char *)pActor->shop_saleitem,szText_shop_saleitem,sizeof(short)*6),db_escape((const char *)pActor->neq_crit,szText_neq_crit,sizeof(char)*6),pActor->mapcallfree,db_escape((const char *)pActor->wishingid,szText_wishingid,sizeof(short)*9),pActor->wishingday,db_escape((const char *)pActor->wishingopen,szText_wishingopen,sizeof(char)*9),pActor->wishingcd,pActor->shape_bag,db_escape((const char *)pActor->student,szText_student,sizeof(int)*30),db_escape((const char *)pActor->te_award,szText_te_award,sizeof(char)*10),db_escape((const char *)pActor->te_awarded,szText_te_awarded,sizeof(char)*10),pActor->te_shop,pActor->act02_state,pActor->act03_state,db_escape((const char *)pActor->act04_state,szText_act04_state,sizeof(char)*128),db_escape((const char *)pActor->act04_value,szText_act04_value,sizeof(int)*40),db_escape((const char *)pActor->act05_item,szText_act05_item,sizeof(short)*6),pActor->act05_isbuy,pActor->act05_buynum,pActor->act05_xw,pActor->act05_upnum,pActor->act05_upstamp,pActor->act08_state,pActor->fs_nodeid,db_escape((const char *)pActor->fs_awardkind,szText_fs_awardkind,sizeof(int)*14),db_escape((const char *)pActor->fs_awardnum,szText_fs_awardnum,sizeof(int)*14),pActor->fs_awardfday,db_escape((const char *)pActor->fs_weekkind,szText_fs_weekkind,sizeof(int)*5),db_escape((const char *)pActor->fs_weeknum,szText_fs_weeknum,sizeof(int)*5),pActor->fs_fweek,db_escape((const char *)pActor->girlshop,szText_girlshop,sizeof(short)*9),pActor->girlshop_buy,pActor->girlshop_fday,db_escape((const char *)pActor->maidname,szText_maidname,0));
	if( fp )
	{
		fprintf( fp, "%s;\n", szSQL );
	}
