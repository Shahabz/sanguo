...
int heroinfo_init_auto()
{
	...
	sprintf( szSQL, "select `kind`,`color`,`corps`,`attack_base`,`attack_wash`,`attack_wash_limit`,`defense_base`,`defense_wash`,`defense_wash_limit`,`troops_base`,`troops_wash`,`troops_wash_limit`,`total_wash`,`attack`,`defense`,`troops`,`skillid`,`itemnum`,`sound` from hero;" );
	if( mysql_query( myData, szSQL ) )
	{
		printf( "Query failed (%s)\n", mysql_error(myData) );
		write_gamelog( "%s", szSQL );
	...
	while( ( row = mysql_fetch_row( res ) ) )
	{
		...
		g_heroinfo[kind].config[color].sound = atoi(row[offset++]);
	}
	mysql_free_result( res );
