...
int mapenemyinfo_init_auto()
{
	...
	sprintf( szSQL, "select `kind`,`level`,`shape`,`first_awardgroup`,`awardgroup`,`monsterid0`,`monsterid1`,`monsterid2`,`monsterid3`,`type`,`nameid` from map_enemyinfo;" );
	if( mysql_query( myData, szSQL ) )
	{
		printf( "Query failed (%s)\n", mysql_error(myData) );
		write_gamelog( "%s", szSQL );
	...
	while( ( row = mysql_fetch_row( res ) ) )
	{
		...
		g_enemyinfo[kind].shape = atoi(row[offset++]);
		...
		g_enemyinfo[kind].type = atoi(row[offset++]);
		g_enemyinfo[kind].nameid = atoi(row[offset++]);
	}
	mysql_free_result( res );
	return 0;
}
