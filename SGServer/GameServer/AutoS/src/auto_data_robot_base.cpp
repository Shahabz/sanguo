...
int robotbase_init_auto()
{
	...
	sprintf( szSQL, "select `level`,`minute`,`actorexp_add`,`actorlevel_max`,`silver_add`,`silver_max`,`wood_add`,`wood_max`,`food_add`,`food_max`,`iron_add`,`iron_max`,`soldiers_add0`,`soldiers_add1`,`soldiers_add2`,`soldiers_max0`,`soldiers_max1`,`soldiers_max2`,`herokind0`,`herokind1`,`herokind2`,`herokind3`,`herokind4`,`herokind5`,`herokind6`,`herokind7`,`heroup_max` from robot_base;" );
	if( mysql_query( myData, szSQL ) )
	{
		printf( "Query failed (%s)\n", mysql_error(myData) );
		write_gamelog( "%s", szSQL );
	...
	while( ( row = mysql_fetch_row( res ) ) )
	{
		...
		g_robot_base[level].minute = atoi(row[offset++]);
		g_robot_base[level].actorexp_add = atoi(row[offset++]);
		...
		g_robot_base[level].soldiers_add[0] = atoi(row[offset++]);
		g_robot_base[level].soldiers_add[1] = atoi(row[offset++]);
		g_robot_base[level].soldiers_add[2] = atoi(row[offset++]);
		g_robot_base[level].soldiers_max[0] = atoi(row[offset++]);
		g_robot_base[level].soldiers_max[1] = atoi(row[offset++]);
		g_robot_base[level].soldiers_max[2] = atoi(row[offset++]);
