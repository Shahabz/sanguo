...
{
	SLK_NetC_QuestTalkNext Value = {0};
	int tmpsize = size;
	char *ptr = data;

	struct_NetC_QuestTalkNext_recv( &ptr, &tmpsize, &Value );

	proc_questtalknext_S( client_index, &Value );
...
{
	SLK_NetC_RankAsk Value = {0};
	int tmpsize = size;
	char *ptr = data;

	struct_NetC_RankAsk_recv( &ptr, &tmpsize, &Value );

	proc_ranklistask_S( client_index, &Value );
...
int proc_command_S( int client_index, short cmd, char *ptr, int tmpsize, int exec_code )
{
	...
	case CMDC_HEROGUARDSORT:
		...
		break;
	case CMDC_QUESTTALKNEXT:
		netrecv_questtalknext_S( client_index, ptr, tmpsize );
		break;
