...
int army_group_load_auto( LPCB_GETARMYGROUP pCB_GetArmyGroup, LPCB_LOADARMYGROUP pCB_LoadArmyGroup, const char *pTab )
{
	...
	sprintf( szSQL, "select `index`,`id`,`state`,`statetime`,`stateduration`,`type`,`from_type`,`from_id`,`from_index`,`from_posx`,`from_posy`,`from_nation`,`from_helpnum`,`to_type`,`to_id`,`to_index`,`to_posx`,`to_posy`,`to_nation`,`to_helpnum`,`leader_index`,`attack_armyindex`,`defense_armyindex`,`attack_helpernum`,`defense_helpernum` from %s ", pTab );
	...
	while( ( row = mysql_fetch_row( res ) ) )
	{
		...
		memcpy( pArmyGroup->attack_helpernum, row[offset++], sizeof(int)*12 );
		memcpy( pArmyGroup->defense_helpernum, row[offset++], sizeof(int)*12 );
...
int army_group_save_auto( ArmyGroup *pArmyGroup, const char *pTab, FILE *fp )
{
	...
	char szText_attack_helpernum[sizeof(int)*12*2+1]={0};
	char szText_defense_helpernum[sizeof(int)*12*2+1]={0};
RE_ARMYGROUP_UPDATE:
	sprintf( szSQL, "REPLACE INTO %s (`index`,`id`,`state`,`statetime`,`stateduration`,`type`,`from_type`,`from_id`,`from_index`,`from_posx`,`from_posy`,`from_nation`,`from_helpnum`,`to_type`,`to_id`,`to_index`,`to_posx`,`to_posy`,`to_nation`,`to_helpnum`,`leader_index`,`attack_armyindex`,`defense_armyindex`,`attack_helpernum`,`defense_helpernum`) Values('%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%s','%s','%s','%s')",pTab,pArmyGroup->index,pArmyGroup->id,pArmyGroup->state,pArmyGroup->statetime,pArmyGroup->stateduration,pArmyGroup->type,pArmyGroup->from_type,pArmyGroup->from_id,pArmyGroup->from_index,pArmyGroup->from_posx,pArmyGroup->from_posy,pArmyGroup->from_nation,pArmyGroup->from_helpnum,pArmyGroup->to_type,pArmyGroup->to_id,pArmyGroup->to_index,pArmyGroup->to_posx,pArmyGroup->to_posy,pArmyGroup->to_nation,pArmyGroup->to_helpnum,pArmyGroup->leader_index,db_escape((const char *)pArmyGroup->attack_armyindex,szText_attack_armyindex,sizeof(int)*128),db_escape((const char *)pArmyGroup->defense_armyindex,szText_defense_armyindex,sizeof(int)*128),db_escape((const char *)pArmyGroup->attack_helpernum,szText_attack_helpernum,sizeof(int)*12),db_escape((const char *)pArmyGroup->defense_helpernum,szText_defense_helpernum,sizeof(int)*12));
	if( fp )
	{
		fprintf( fp, "%s;\n", szSQL );
	}
