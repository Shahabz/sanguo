...
int system_askinfo( int actor_index, int msgid, char *pstr, int *pvalue )
{
	...
	case ASKINFO_BUYRES:
		...
		break;
	case ASKINFO_EVERYDAY_EVENT: // 政务
		if ( pvalue[0] == 0 )
		{
		...
		break;
	case ASKINFO_EVERYDAY_QUEST: // 每日
		if ( pvalue[0] == 0 )
		{
	...
	case ASKINFO_COLISEUM: // 竞技场
		...
		{ // 战报
			coliseum_loglist( actor_index, 0 );
		}
		else if ( pvalue[0] == 6 )
		{ // 观战
			coliseum_fight_read( actor_index, pvalue[1], 0 );
		}
		else if ( pvalue[0] == 7 )
		{ // 精彩对决
			coliseum_loglist( actor_index, 1 );
		}
		else if ( pvalue[0] == 8 )
		{ // 重置今日次数
		...
		break;
	case ASKINFO_EVERYDAY_SIGNIN: // 签到
		if ( pvalue[0] == 0 )
		{
