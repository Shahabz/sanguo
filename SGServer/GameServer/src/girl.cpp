...
// 玩家获得女将
int girl_getgirl( City *pCity, int kind, int color, char path )
{
	if ( !pCity )
		return -1;
	if ( kind <= 0 || kind >= g_girlinfo_maxnum )
	...
	if ( pHasGirl && pHasGirl->kind > 0 )
	{
		...
		// 统计log
		wlog( 0, LOGOP_GIRL, path, kind, config->soulchange, 0, pCity->actorid, city_mainlevel( pCity->index ) );
		return -1;
	}

	memset( &pCity->girl[kind], 0, sizeof( Girl ) );
	pCity->girl[kind].actorid = pCity->actorid;
	pCity->girl[kind].kind = kind;
	pCity->girl[kind].color = color;
	...
	// 统计log
	wlog( 0, LOGOP_GIRL, path, kind, 0, 0, pCity->actorid, city_mainlevel( pCity->index ) );
	return 0;
}
...
void girl_gm_getall( City *pCity )
{
	...
	{
		girl_getgirl( pCity, kind, 1, PATH_GM );
	}
}
