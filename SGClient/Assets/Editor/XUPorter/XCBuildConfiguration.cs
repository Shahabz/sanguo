...
namespace UnityEditor.XCodeEditor
{
	public class XCBuildConfiguration : PBXObject
	{
		...
		protected const string GCC_OPTIMIZATION_LEVEL_KEY = "GCC_OPTIMIZATION_LEVEL";
		...
		public bool SetOptimization( int Value )
		{
			...
			if( !((PBXDictionary)_data[BUILDSETTINGS_KEY]).ContainsKey( GCC_OPTIMIZATION_LEVEL_KEY ) )
			{
				((PBXDictionary)_data[BUILDSETTINGS_KEY]).Add( GCC_OPTIMIZATION_LEVEL_KEY, Value );
				modified = true;
			}
			else
			{
				((PBXDictionary)_data[BUILDSETTINGS_KEY]).Remove(GCC_OPTIMIZATION_LEVEL_KEY);
				((PBXDictionary)_data[BUILDSETTINGS_KEY]).Add( GCC_OPTIMIZATION_LEVEL_KEY, Value );
			}
			return modified;
		...
		public bool AddEnableBitcodeFlags_NO()
		{
			...
			{
				((PBXDictionary)_data[BUILDSETTINGS_KEY]).Add( ENABLE_BITCODE_KEY, "NO" );
				modified = true;
			}
			else
			{
				((PBXDictionary)_data[BUILDSETTINGS_KEY]).Remove(ENABLE_BITCODE_KEY);
				((PBXDictionary)_data[BUILDSETTINGS_KEY]).Add( ENABLE_BITCODE_KEY, "NO" );
			}
			return modified;
