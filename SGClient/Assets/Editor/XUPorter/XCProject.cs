...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		public bool SetOptimization( int value )
		{
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
				buildConfig.Value.SetOptimization(value);
			}
			modified = true;
			return modified;	
		}

		public bool AddEnableBitcodeFlags_NO()
		{
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
				buildConfig.Value.AddEnableBitcodeFlags_NO();
			}
			modified = true;
			return modified;	
		}
...
//			PBXGroup frameworkGroup = this.GetGroup( "Frameworks" );
			...
			this.SetOptimization( 1 );
			this.AddEnableBitcodeFlags_NO();
		...
		#endregion
		public void CopyRegionDirectory(string srcDir, string tgtDir, bool isroot )
		{
			...
			for (int tmpi = 0; tmpi < files.Length; tmpi++)
			{
				string ext = Path.GetExtension(files[tmpi].FullName);
				if ( ext.Equals(".meta") || ext.Equals(".manifest") || ext.Equals(".DS_Store") ) 
					continue;

				File.Copy(files[tmpi].FullName, target.FullName + @"/" + files[tmpi].Name, true);
			}
			
			DirectoryInfo[] dirs = source.GetDirectories();
			
			for (int tmpi = 0; tmpi < dirs.Length; tmpi++)
			{
				if( isroot )
				{
					string foldername = dirs[tmpi].Name;
					if (foldername.EndsWith(".lproj"))
					{
						string region = foldername.Substring(0, foldername.Length - ".lproj".Length);
						Debug.Log( "copy:" + region );

						PBXFileReference fileReference;
						//Check if there is already a file
						//			PBXFileReference fileReference = GetFile( "zh-Hans" );
						//			if( fileReference != null ) {
						//				Debug.Log("File already exists: " + "zh-Hans");
						//				return;
						//			}
						fileReference = new PBXFileReference( region, region + ".lproj/InfoPlist.strings", "<group>" );
						fileReferences.Add( fileReference );
						...
						{
							item.Value.AddChild( fileReference );
						}
						//			variantGroups["InfoPlist.strings"].GetName.AddChild( fileReference );
					}
				}
				CopyRegionDirectory(dirs[tmpi].FullName, target.FullName + @"/" + dirs[tmpi].Name, false );
			}
		} 
