...
class YlyRichTextParser
{
	...
	//201: <url=click_arg>类型;                         //超链接; click_arg参数不能带字符'>'
	//202: <u>类型;                                     //下划线;
	//203: </url>类型;                                  //超链接终止符;
	//204: </color>类型;                                //颜色终止符;
	//205: <r>类型;                                     //回车;
    ...
    int[] _mask;
	Dictionary<int, string> _parms;
	Dictionary<string, int> _preLoadAsset;
	...
	public string Parse(string str, out int[] mask, out Dictionary<int, string> param, out Dictionary<string, int> preLoadAsset, bool enableRichText)
    {
    ...
    void ParseSingle(string str, string rex, int ibase)
    {
		string arg;
