...
public class UpdateManager : MonoBehaviour
{
    ...
    {
        // 创建启动日志
        LogUtil.GetInstance().WriteGame( "------------------ Update Start ------------------" );
    ...
    IEnumerator OnExtractResource()
    {
        ...
        // ------------------------------ 文件列表 -------------------------------------
        // 有文件列表，先删除，再创建，保持干净
        string sourceFile = sourcePath + PathUtil.updateFileListName();
        string targetFile = targetPath + PathUtil.updateFileListName();
    ...
    IEnumerator OnUpdateResource()
    {
        ...
        // 读取本地文件列表
        string[] szlocalFileList = File.ReadAllLines( PathUtil.LocalPath() + PathUtil.updateFileListName() );
        ...
        // 获取服务器的更新文件地址
        // 先取cfg文件，没有文件才取php
        LogUtil.GetInstance().WriteGame( url + PathUtil.updateFileName() + ".cfg" );
        WWW www = new WWW( url + PathUtil.updateFileName() + ".cfg" );
        yield return www;
        if ( www.error != null )
        {
            // 取php
            LogUtil.GetInstance().WriteGame( url + PathUtil.updateFileName() + ".php" );
            www = new WWW( url + PathUtil.updateFileName() + ".php" );
            yield return www;
            if ( www.error != null )
            {
                // 更新地址错误也进游戏
        ...
        // 先下载文件列表到本地
        www = new WWW( updateResUrl + "/" + PathUtil.updateFileListName() );
        yield return www;
        if ( www.error != null )
        {
        ...
        File.WriteAllBytes( PathUtil.PatchDownloadPath() + PathUtil.updateFileListName(), www.bytes );

        // 比较web下载下来的文件列表和本地的两个文件列表的差异数据
        m_downloadsize_total = 0;
        m_szNeedUpdateFileDict = new Dictionary<string, DownLoadFile>();
        string[] szUpdateFileList = File.ReadAllLines( PathUtil.PatchDownloadPath() + PathUtil.updateFileListName() );
    ...
    public void OnInited()
    {
        ...
        GameObject.FindWithTag( "GameManager" ).GetComponent<GameManager>().Initialize();

        LogUtil.GetInstance().WriteGame( "------------------ Update OnInited ------------------" );
    }
}
