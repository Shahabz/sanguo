	            ...
	            if( Input.GetMouseButtonDown(0) )
	            {
	                ...
	                if( _Lock == false )
	                {
	                    ...
	                    if ( hit.collider != null )
	                    {
	                        ...
	                        _DragObj = hit.collider.transform.GetComponent<Camera2dDragable>();
	                        if ( _DragObj != null )
	                            _DragState = 1;
	                    }
	                }
				...
				if( _Lock == false )
				{
					...
					{			
						...
						if( Input.GetTouch(0).phase == TouchPhase.Moved || Input.GetTouch(1).phase == TouchPhase.Moved )
						{
							...
							// 移动缩放
							ZoomMove( leng2 - leng1, new Vector3( _TargetPos.x, _TargetPos.y, 0 ), ( tempPosition1 - _OldPosition1 ) + ( tempPosition2 - _OldPosition2 ) );
...
//#endif
        ...
        if( tag ==  "MainCamera" )
        {
            ...
            //RecUtil.GetInstance().WriteCamera( pos.x, pos.y, pos.z );
        }
    ...
    // 还原摄像机缩放
    public void RecoverDefalutSize()
    {
        _Camera.orthographicSize = defaultSize;
        RefreshSubCamera();
        OnTween();
    ...
    protected virtual void OnTouchRelease( Vector2 screenPos )
    {
		if( enableTouchEffect && _OnTouchEffect != null)
        {
            ...
            Vector3 pos = eye.uiManager.GetUICamera().ScreenToWorldPoint( screenPos );
            pos.z = -5.12f;
